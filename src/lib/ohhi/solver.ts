...
export class Solver {
	static solve(board: Board): Board {
		...
		if (!new_board.is_solution()) throw new NoSolutionFound('No solution found');
					...
					case Tile.Yellow:
						...
						if (yellow_count == 2) {
							if (first_yellow_idx > 0 && first_yellow_idx < new_board.size)
								new_board.state[row_idx][first_yellow_idx - 1] = Tile.Blue;
							if (first_yellow_idx + 2 > 0 && first_yellow_idx + 2 < new_board.size)
								new_board.state[row_idx][first_yellow_idx + 2] = Tile.Blue;
						}
						break;
					case Tile.Blue:
						...
						if (blue_count == 2) {
							if (first_blue_idx > 0 && first_blue_idx < new_board.size)
								new_board.state[row_idx][first_blue_idx - 1] = Tile.Yellow;
							if (first_blue_idx + 2 > 0 && first_blue_idx + 2 < new_board.size)
								new_board.state[row_idx][first_blue_idx + 2] = Tile.Yellow;
						}
						break;
					case Tile.Empty:
						first_yellow_idx = -1;
	...
	static fill_middle_tiles(board: Board): Board {
		...
		for (let row_idx = 0; row_idx < new_board.size; row_idx++) {
			for (let col_idx = 0; col_idx < new_board.size - 2; col_idx++) {
				if (
					new_board.at(row_idx, col_idx) == Tile.Yellow &&
					new_board.at(row_idx, col_idx + 2) == Tile.Yellow
				) {
					new_board.state[row_idx][col_idx + 1] = Tile.Blue;
				} else if (
					new_board.at(row_idx, col_idx) == Tile.Blue &&
					new_board.at(row_idx, col_idx + 2) == Tile.Blue
				) {
	...
	static fill_uneven_rows(board: Board): Board {
		const new_board = board.copy();
		for (let row_idx = 0; row_idx < new_board.size; row_idx++) {
			const { yellow_count, blue_count } = count_non_empty_tiles(new_board.state[row_idx]);
			if (yellow_count == new_board.size / 2) {
				for (let col_idx = 0; col_idx < new_board.size; col_idx++) {
					if (new_board.state[row_idx][col_idx] == Tile.Empty)
	...
	static fill_different_rows(board: Board): Board {
		...
		for (let row1_idx = 0; row1_idx < new_board.size; row1_idx++) {
			if (count_empty_tiles(new_board.state[row1_idx]) != 2) continue;

			for (let row2_idx = 0; row2_idx < new_board.size; row2_idx++) {
				if (row1_idx == row2_idx || count_empty_tiles(new_board.state[row2_idx]) != 0) continue;
				if (!non_empty_elems_are_equal(new_board.state[row1_idx], new_board.state[row2_idx]))
					continue;
				for (let col_idx = 0; col_idx < new_board.size; col_idx++) {
					if (new_board.state[row1_idx][col_idx] != Tile.Empty) continue;
					if (new_board.state[row2_idx][col_idx] == Tile.Blue)
						new_board.state[row1_idx][col_idx] = Tile.Yellow;
					else if (new_board.state[row2_idx][col_idx] == Tile.Yellow)
						new_board.state[row1_idx][col_idx] = Tile.Blue;
				}
			}
		}
		return new_board;
