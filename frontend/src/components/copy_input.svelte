<script lang="ts">
	...
	async function copyContent(): Promise<void> {
		try {
            ...
            setTimeout(() => {showCopiedTooltip = false}, 800);
		} catch (err) {
...
<div class="max-w-md flex">
	...
	>
    <div
        class="hover:translate-y-[-3px] duration-200 relative"
    >
        {#if showCopiedTooltip}
            <span transition:fade={{duration: 200}} class="absolute top-6 right-[-32px] bg-neutral-700 text-neutral-100 px-3 py-1 rounded-md">COPIED</span>
        {/if}
        <svg
